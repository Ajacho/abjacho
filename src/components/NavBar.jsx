    ...
    return (
        <nav className="fixed top-0 left-0 z-50 w-full shadow-md bg-gradient-to-l from-[#063064] to-[#0a192fd7]">
            <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
                <a href="#" className="text-2xl font-bold hover:text-yellow-400 ">Andrea B Jacho</a>
                ...
                <ul className="hidden md:flex gap-6 text-sm">
                    {desktopNavItems.map((item) => (
                        <li 
                            ...
                            <Link to={item.toLowerCase()} smooth={true} duration={500} className="cursor-pointer">{item}</Link>
                        </li>
                    ))}
                ...
                <ul className="text-white flex gap-4">
                    {mobileNavItems.map((item) => (
                        <li key={item.name} className="list-none hover:text-yellow-400 transition-colors duration-300">
